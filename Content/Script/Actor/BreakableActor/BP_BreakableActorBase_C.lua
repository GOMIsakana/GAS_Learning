...
local function SpawnActorTimer(Self, LootArray)
    ...
    for LootIndex = 1, LootArray:Length() do
        print("SpawnActorTimer Looping, Index is: " .. LootIndex)
        local Loot = LootArray:Get(LootIndex)
        local SpawnedActor = World:SpawnActorDefered(Loot.ItemClass, Self:GetTransform(), AlwaysSpawn, Self, Self, "Actor.BP_AuraEffectActorBase_C")    -- 生成Actor
